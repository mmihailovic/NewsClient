...
<div v-if="tagovi && tagovi.length > 0">
        ...
        <div class="tag-container">
          <p v-for="(tag,index) in tagovi" :key="index" class="tag">{{ tag.kljucnaRec }} <button @click="removeItem(index)" class="tag-remove">x</button></p>
        </div>
</div>
...
export default {
    ...
    created() {
        const news = JSON.parse(this.$route.query.news);
        this.id = news.id;
        this.tekst = news.text;
        this.naslov = news.title;
        this.kategorija = news.category;
    ...
    methods: {
        ...
        dodajTag() {
            this.tagovi.push({kljucnaRec:this.tag, vest_id:this.id})
            this.tag=''
        },
        kreirajVest() {
            this.$axios.post('/api/news/' + this.id, {
                naslov: this.naslov,
                tekst: this.tekst,
            ...
            .then(() => {
                ...
                console.log(this.tagovi)
                this.$axios.post('/api/tag/news/' + this.id,this.tagovi,{headers})
                .then(() => {
